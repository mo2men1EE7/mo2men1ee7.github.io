...
function toggleGlass() {
  ...
  /* ── Freeze & eject Clippy BEFORE attribute change so CSS transitions can't react ── */
  stopClippyFloat();
  ...
  if (wrap && wrap.classList.contains('clippy-in')) {
    ...
    /* Lock current position — kill ALL transitions/animations on the element */
    var rect = wrap.getBoundingClientRect();
    wrap.style.cssText = 'position:fixed;bottom:' + (window.innerHeight - rect.bottom) + 'px;right:32px;z-index:99998;display:flex;flex-direction:column;align-items:center;gap:6px;transition:none;animation:none;';
  ...
  /* Now safe to change the attribute */
...
function toggleTheme() {
  stopClippyFloat(); /* stop float BEFORE attribute change */
  var html = document.documentElement;
  ...
  refreshBadges();
  updateTagIcons();
  /* Dismiss Clippy cleanly on theme switch — freeze before theme attr changes */
  stopClippyFloat();
  var wrapT = document.getElementById('clippy-wrap');
  if (wrapT && wrapT.classList.contains('clippy-in')) {
    var rectT = wrapT.getBoundingClientRect();
    wrapT.style.cssText = 'position:fixed;bottom:' + (window.innerHeight - rectT.bottom) + 'px;right:32px;z-index:99998;display:flex;flex-direction:column;align-items:center;gap:6px;transition:none;animation:none;';
    wrapT.offsetHeight;
    var startT = null; var durT = 400;
    (function step(ts) {
      if (!startT) startT = ts;
      var p = Math.min((ts - startT) / durT, 1);
      wrapT.style.opacity = 1 - p;
      wrapT.style.transform = 'translateY(' + (-p * 40) + 'px)';
      if (p < 1) { requestAnimationFrame(step); } else { wrapT.style.display = 'none'; }
    })(performance.now());
  }
}
...
function stopClippyFloat() {
  ...
  if (wrap) {
    ...
    wrap.offsetHeight; /* force reflow — flush transform before next paint */
